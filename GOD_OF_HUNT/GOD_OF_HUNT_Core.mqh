...
//==================== اندازه پنل روی مانیتور ====================
// این بخش هیچ ربطی به تشخیص ندارد و mksim.py آن را از شبیه ساز حذف می‌کند
// (بین همین دو نشانه)، وگرنه شبیه ساز به توابع رابط کاربری متاتریدر نیاز
// پیدا می‌کرد که هیچ کدام در تشخیص نقشی ندارند.
//<<< UI-LAYOUT-BEGIN
input group "=== اندازه پنل روی مانیتور (فقط ظاهر) ==="
// هیچ اثری روی تشخیص ندارد و عمدا در CoreConfigSignature نیامده است.
input int    UIScalePercent        = 0;    // 0 = خودکار از DPI مانیتور، وگرنه درصد دستی (مثلا 150)

//+------------------------------------------------------------------+
// ضریب بزرگنمایی پنل.
//
// مختصات آبجکت های پنل (XDISTANCE, YDISTANCE, XSIZE, YSIZE) بر حسب پیکسل
// خام است، ولی متاتریدر OBJPROP_FONTSIZE را بر حسب «پوینت» می‌گیرد و با
// DPI مانیتور به پیکسل تبدیل می‌کند. روی یک لپ تاپ ۱۵ اینچ با اسکیل ۱۲۵ یا
// ۱۵۰ درصد ویندوز، متن ۱.۲۵ تا ۱.۵ برابر بزرگ می‌شود ولی کادر دکمه و فاصله
// ردیف ها همان قدر قبلی می‌ماند — نتیجه اش همان چیزی است که دیده می‌شود:
// متن دکمه نصفه می‌شود و ردیف های زیر دکمه ها روی هم می‌افتند.
//
// این تابع دقیقا همان اختلاف را برمی‌گرداند تا هندسه هم قد فونت شود.
// فونت عمدا ضرب نمی‌شود؛ متاتریدر خودش آن را بزرگ کرده و ضرب دوباره
// باعث می‌شد متن دو برابر شود.
double UiScale()
{
   if(UIScalePercent > 0) return UIScalePercent / 100.0;

   int dpi = (int)TerminalInfoInteger(TERMINAL_SCREEN_DPI);
   if(dpi <= 0) return 1.0;          // ترمینال DPI را نداد، همان حالت قبلی

   double s = dpi / 96.0;            // 96 = مانیتور معمولی، اسکیل ۱۰۰ درصد
   if(s < 1.0) s = 1.0;              // کوچکتر از حالت پایه نشود
   if(s > 3.0) s = 3.0;              // سقف، تا یک DPI عجیب پنل را از چارت بیرون نبرد
   return s;
}

// یک عدد پیکسلیِ «اسکیل ۱۰۰ درصد» را به پیکسل این مانیتور تبدیل می‌کند.
int UiPx(int px) { return (int)MathRound(px * UiScale()); }

//+------------------------------------------------------------------+
// چیدمان پنل مشترک بین اندیکاتور چارت و نسخه بک تست. همه اعداد بر حسب
// «پیکسل در اسکیل ۱۰۰ درصد» هستند و موقع رسم از UiPx رد می‌شوند.
#define UI_MARGIN     10   // فاصله از لبه چپ و بالای چارت
#define UI_BTN_W     140   // عرض دکمه ها (جا برای «[x] TICK FRACTAL»)
#define UI_BTN_H      22   // ارتفاع دکمه ها
#define UI_BTN_ROW    30   // فاصله ردیف تا ردیف دکمه ها
...
#define UI_FONT_TXT   10   // فونت خطوط فراکتال و سشن

// مختصات ردیف row ام دکمه ها (۰ تا ۵)
int UiBtnY(int row)  { return UiPx(UI_MARGIN + UI_BTN_ROW * row); }

// مختصات ردیف row ام متن های زیر دکمه ها (۰ = تایمر کندل)
int UiTextY(int row) { return UiPx(UI_MARGIN + UI_BTN_ROW * UI_BTN_ROWS + UI_TEXT_ROW * row); }

// لبه چپ ستون دکمه ها و لبه چپ لیست بازشو
int UiPanelX()       { return UiPx(UI_MARGIN); }
int UiListX()        { return UiPx(UI_MARGIN + UI_BTN_W + UI_BTN_GAP); }
//>>> UI-LAYOUT-END
