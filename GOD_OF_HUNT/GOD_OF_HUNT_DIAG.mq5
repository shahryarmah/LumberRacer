//+------------------------------------------------------------------+
//|                                       GOD_OF_HUNT_DIAG.mq5   v1.16   |
//|  نسخهٔ اندازه‌گیری. رفتارش دقیقا مثل 1.15 است، فقط زمان هر بخش را       |
//|  می‌سنجد و در تب Experts می‌نویسد. برای کار روزمره استفاده نشود.        |
...
#property description "GOD_OF_HUNT DIAG - اندازه گیری زمان بخش ها"
...
//================== ابزار سنجش، فقط در همین نسخه ==================
//
// چرا این شکلی: متاتریدر فقط می‌گوید «indicator is too slow, N ms» ولی
// نمی‌گوید آن N میلی‌ثانیه کجا رفت. اینجا هر هندلر از ابتدا تا انتها زمان
// می‌خورد و داخل ProcessIndicator هر بخش جدا می‌شود.
//
// «فاصله» مهم‌ترین عدد است: فاصلهٔ ورود این اجرا تا ورود اجرای قبلی. اگر
// خودِ اجرا کوتاه باشد ولی فاصله چند ثانیه، یعنی ترد چارت جای دیگری بلاک
// شده و کد ما مقصر نیست. اگر اجرا خودش طولانی باشد، مقصر ماییم.
input group "=== تشخیص علت فریز (فقط نسخهٔ DIAG) ==="
input bool DiagEnabled       = true;   // نوشتن گزارش در تب Experts
input int  DiagSlowMs        = 100;    // هر اجرای کندتر از این، یک خط گزارش
input int  DiagSummarySec    = 30;     // هر چند ثانیه یک خط خلاصه
input bool DiagThrottleTicks = false;  // تست گام ۱: اجرای کامل فقط با کندل جدید

#define DG_MAX 12

string dgWho = "";
ulong  dgEnterUs = 0, dgPrevEnter = 0, dgMarkUs = 0;
string dgSecName[DG_MAX];
ulong  dgSecUs[DG_MAX];
int    dgSecN = 0;
bool   dgInside = false;

ulong  dgCalls = 0, dgSumUs = 0, dgWorstUs = 0, dgWorstGapUs = 0;
string dgWorstLine = "";
ulong  dgLastSummaryUs = 0;
int    dgSlowCount = 0;

datetime lastTickBar = 0;    // فقط وقتی DiagThrottleTicks روشن باشد

void DgEnter(string who)
{
   if(!DiagEnabled || dgInside) return;   // تودرتو شمرده نشود
   dgInside    = true;
   dgWho       = who;
   dgPrevEnter = dgEnterUs;
   dgEnterUs   = GetMicrosecondCount();
   dgMarkUs    = dgEnterUs;
   dgSecN      = 0;
}

void DgLap(string name)
{
   if(!DiagEnabled || !dgInside || dgSecN >= DG_MAX) return;
   ulong now = GetMicrosecondCount();
   dgSecName[dgSecN] = name;
   dgSecUs[dgSecN]   = now - dgMarkUs;
   dgSecN++;
   dgMarkUs = now;
}

string DgMs(ulong us) { return DoubleToString(us / 1000.0, 1); }

void DgLeave()
{
   if(!DiagEnabled || !dgInside) return;
   dgInside = false;

   ulong now = GetMicrosecondCount();
   ulong dur = now - dgEnterUs;
   ulong gap = (dgPrevEnter > 0) ? (dgEnterUs - dgPrevEnter) : 0;

   dgCalls++;
   dgSumUs += dur;
   if(gap > dgWorstGapUs) dgWorstGapUs = gap;

   string line = dgWho + " " + DgMs(dur) + "ms  فاصله=" + DgMs(gap) +
                 "ms  آبجکت=" + IntegerToString(ObjectsTotal(0)) + "  |";
   for(int i = 0; i < dgSecN; i++)
      line += " " + dgSecName[i] + "=" + DgMs(dgSecUs[i]);

   if(dur > dgWorstUs) { dgWorstUs = dur; dgWorstLine = line; }

   if(DiagSlowMs > 0 && dur >= (ulong)DiagSlowMs * 1000)
   {
      dgSlowCount++;
      Print("GOH-DIAG کند | ", line);
   }

   if(DiagSummarySec > 0 &&
      (dgLastSummaryUs == 0 || now - dgLastSummaryUs > (ulong)DiagSummarySec * 1000000))
   {
      dgLastSummaryUs = now;
      Print("GOH-DIAG خلاصه | اجرا=", dgCalls,
            "  میانگین=", DgMs(dgCalls > 0 ? dgSumUs / dgCalls : 0),
            "ms  بدترین=", DgMs(dgWorstUs),
            "ms  بیشترین فاصله=", DgMs(dgWorstGapUs),
            "ms  تعداد کند=", dgSlowCount);
      Print("GOH-DIAG بدترین اجرا | ", dgWorstLine);
   }
}
...
void UpdateLevels(bool active)
{
   ...
   for(int i = 0; i < SESSION_COUNT; i++)
   {
      ...
      if(!on)
      {
         if(lvlOnDrawn[p]) { DeleteLevelGroup(idH); DeleteLevelGroup(idL); }
         lvlOnDrawn[p]   = false;
         lvlSessDrawn[i] = 0;
         continue;
      }
      ...
      if(!SessionWindowToday(i, fromSrv, toSrv))
      {
...
{
   // سطوح افقی پیشوند جدا دارند: gohlv_DH / gohlv_DL / gohlv_S<i>H ...
...
{
   return "GOH_BtnPatHead_" + IntegerToString(ChartID());
}
...
// پوشش هندلرها: بدنه ها return زودهنگام دارند، پس اندازه گیری بیرونشان است.
int OnInit()
{
   DgEnter("init");
   int r = OnInitBody();
   if(DiagEnabled)
      Print("GOH-DIAG شروع | چارت=", TFToStr((ENUM_TIMEFRAMES)Period()),
            "  STRUCT=", TFToStr(StructureTF), "  TRIG=", TFToStr(TriggerTF),
            "  ENTRY=", TFToStr(EntryTF),
            "  DPI=", TerminalInfoInteger(TERMINAL_SCREEN_DPI),
            "  آبجکت=", ObjectsTotal(0),
            "  عرض چارت=", ChartGetInteger(0, CHART_WIDTH_IN_PIXELS),
            "  ارتفاع=", ChartGetInteger(0, CHART_HEIGHT_IN_PIXELS));
   DgLeave();
   return r;
}
...
void OnChartEventBody(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   string chartIDStr    = IntegerToString(ChartID());
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      ...
      if(StringFind(sparam, "GOH_BtnPat") == 0)
      {
         ...
         if(p >= 0 && p < PATTERN_COUNT)
         {
            ...
            DrawPatternButton(p);
            DrawPatternHeadButton();   // شمارندهٔ روی سرصفحه هم عوض می‌شود
            SaveState();
...
void ProcessIndicator()
{
   ...
   DgLap(fullRedraw ? "wipeFULL" : "wipeLive");
...
{
   DgEnter("event");
   OnChartEventBody(id, lparam, dparam, sparam);
   DgLeave();
}
